#!/usr/bin/env python3
"""Tkinter GUI for the Version 5.3 QAOA optimizer.

V5.3 keeps the V5.2 GUI layout and points it at the V5.3 core. The app-level
edits are narrow:

* Imports now point at ``qaoa_optimizer_V5_3_core``.
* The driver class is renamed ``OptimizerGUIV53``.
* The banner / summary strings advertise Version 5.3.
* Internal type hints reference ``QAOAOptimizerV53``.
...
All V5.3-specific algorithmic fixes (including the concentration polish pass
documented in the core) live in ``qaoa_optimizer_V5_3_core.py`` and are
...
from qaoa_optimizer_V5_3_core import OptimizationCancelled, OptimizationError, QAOAOptimizerV53


class OptimizerGUIV53:
    ...
    def _setup_ui(self):
        ...
        ttk.Label(
            header_frame,
            text="Version 5.3 adds an exact-mode concentration polish on top of the V5.2 search fixes and keeps the three-way Risk/Return chart",
    ...
    def _save_risk_return_sharpe_chart(self, optimizer: QAOAOptimizerV53, out_dir: Path) -> Path:
    ...
    def _save_risk_return_qubo_chart(self, optimizer: QAOAOptimizerV53, out_dir: Path) -> Path:
        classical_cmap = mcolors.LinearSegmentedColormap.from_list(
            "classical_qubo",
    ...
    def _save_risk_return_qubo_quantum_by_qubo_chart(self, optimizer: QAOAOptimizerV53, out_dir: Path) -> Path:
        classical_cmap = mcolors.LinearSegmentedColormap.from_list(
    ...
    def _save_risk_return_qubo_budget_normalized_chart(self, optimizer: QAOAOptimizerV53, out_dir: Path) -> Path:
        classical_cmap = mcolors.LinearSegmentedColormap.from_list(
    ...
    def _save_solver_comparison_chart(self, optimizer: QAOAOptimizerV53, out_dir: Path) -> Path:
    ...
    def _save_qaoa_history_chart(self, optimizer: QAOAOptimizerV53, out_dir: Path) -> Path | None:
    ...
    def _build_summary_text(self, optimizer: QAOAOptimizerV53, chart_dir: Path) -> str:
        lines = [
    ...
    def _collect_circuit_details(self, optimizer) -> dict:
        probe = QAOAOptimizerV53(str(optimizer.xlsx_path), log_callback=None, progress_callback=None)
        probe.load_input()
        probe.build_qubo()
    ...
    def _show_results_window(self, optimizer: QAOAOptimizerV53):
...
OptimizerGUIV52 = OptimizerGUIV53
OptimizerGUIV51 = OptimizerGUIV53
OptimizerGUIV5 = OptimizerGUIV53


def main():
    root = tk.Tk()
    app = OptimizerGUIV53(root)
    root.mainloop()
    return app
