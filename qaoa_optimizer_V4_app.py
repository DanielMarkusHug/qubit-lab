#!/usr/bin/env python3
"""Tkinter GUI for the Version 4 QAOA optimizer."""
...
from qaoa_optimizer_V4_core import OptimizationCancelled, OptimizationError, QAOAOptimizerV4
...
class OptimizerGUIV3:
    ...
    def _setup_ui(self):
        ...
        ttk.Label(
            header_frame,
            text="Version 4 with adaptive QAOA initialization, more realistic runtime tracking, and deeper diagnostics for portfolio-quality analysis",
    ...
    def _save_risk_return_sharpe_chart(self, optimizer: QAOAOptimizerV4, out_dir: Path) -> Path:
    ...
    def _save_risk_return_qubo_chart(self, optimizer: QAOAOptimizerV4, out_dir: Path) -> Path:
        classical_cmap = mcolors.LinearSegmentedColormap.from_list(
            "classical_qubo",
    ...
    def _save_risk_return_qubo_quantum_by_qubo_chart(self, optimizer: QAOAOptimizerV4, out_dir: Path) -> Path:
        classical_cmap = mcolors.LinearSegmentedColormap.from_list(
    ...
    def _save_risk_return_qubo_budget_normalized_chart(self, optimizer: QAOAOptimizerV4, out_dir: Path) -> Path:
        classical_cmap = mcolors.LinearSegmentedColormap.from_list(
    ...
    def _save_solver_comparison_chart(self, optimizer: QAOAOptimizerV4, out_dir: Path) -> Path:
    ...
    def _save_qaoa_history_chart(self, optimizer: QAOAOptimizerV4, out_dir: Path) -> Path | None:
    ...
    def _build_summary_text(self, optimizer: QAOAOptimizerV4, chart_dir: Path) -> str:
        lines = [
    ...
    def _collect_circuit_details(self, optimizer) -> dict:
        probe = QAOAOptimizerV4(str(optimizer.xlsx_path), log_callback=None, progress_callback=None)
        probe.load_input()
        probe.build_qubo()
    ...
    def _show_results_window(self, optimizer: QAOAOptimizerV4):
