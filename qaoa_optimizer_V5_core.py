#!/usr/bin/env python3
"""
QAOA Optimizer Core - Version 5.

V5 actually applies the optimizer fixes that the V4 docstring promised but V4 never wired up,
and fixes several other bugs that destroyed QAOA convergence / concentration on this problem.
...
All other behaviour is preserved. The class is exposed as ``QAOAOptimizerV5``
and also aliased as ``QAOAOptimizerV4`` / ``QAOAOptimizerV3`` so existing
...
class QAOAOptimizerV5:
    """Application-friendly wrapper around the Version 5 notebook logic."""
    ...
    def build_qubo(self):
        self._ensure_not_stopped()
        self._log("Building Version 5 QUBO problem...")
    ...
    def run_classical_search(self):
        ...
        self._log("Running Version 5 classical heuristic...")
    ...
    def _run_exact_p1_presearch(self, make_qaoa_energy, offset: float, qaoa_progress_start: float, qaoa_opt_end: float):
        ...
        # V5: optionally replace the fixed [-1, +1] gamma grid with an adaptive
        ...
        if bool(getattr(self, "qaoa_exact_p1_presearch_adaptive", True)):
            ...
            self._log(
                "V5 adaptive p=1 presearch gamma window: "
    ...
    def run_qaoa(self):
        ...
        self._log(f"Running Version 5 QAOA (p={self.qaoa_p})...")
        ...
        for stage_index, (active_layers, planned_restart_count) in enumerate(stage_restart_plan, start=1):
            ...
            for restart_idx, (init_type, x0) in enumerate(init_candidates, start=1):
                ...
                # V5: scale-aware COBYLA call. rhobeg of 1.0 (scipy default) is
                ...
                if maxiter_per_param <= 0:
                    maxiter_per_param = 30
                effective_maxiter = max(int(self.qaoa_maxiter), maxiter_per_param * n_params)
                self._log(
    ...
    def generate_results(self):
        self._ensure_not_stopped()
        self._log("Generating Version 5 result tables...")
    ...
    def write_results(self):
        self._ensure_not_stopped()
        self._log("Writing Version 5 results back to Excel...")
        ...
        summary_items = [
            ...
            ("QAOA exact p1 presearch adaptive (V5)", int(bool(getattr(self, "qaoa_exact_p1_presearch_adaptive", False)))),
            ("QAOA exact p1 presearch gamma width factor (V5)", float(getattr(self, "qaoa_exact_p1_presearch_gamma_width_factor", 3.0))),
            ("QAOA COBYLA rhobeg setting (V5)", str(getattr(self, "qaoa_cobyla_rhobeg", "auto"))),
            ("QAOA COBYLA catol (V5)", float(getattr(self, "qaoa_cobyla_catol", 1e-6))),
            ("QAOA gamma scale multiplier (V5)", float(getattr(self, "qaoa_gamma_scale_multiplier", 2.0))),
            ("QAOA maxiter per param (V5)", int(getattr(self, "qaoa_maxiter_per_param", 30))),
...
# V5: class is now QAOAOptimizerV5. Keep V3/V4 aliases so external callers
# (e.g. the V4 app or user scripts that import QAOAOptimizerV3/V4) continue to
# work. The aliases resolve to the same updated V5 class, so importing under
# any of these names picks up the V5 fixes automatically.
QAOAOptimizerV4 = QAOAOptimizerV5
QAOAOptimizerV3 = QAOAOptimizerV5
