...
class QAOAOptimizerV3:
    ...
    def _base_qaoa_init(self, layers: int) -> np.ndarray:
        gamma_init = np.linspace(0.30, 0.60, layers)
        beta_init = np.linspace(0.40, 0.20, layers)
        return np.concatenate([gamma_init, beta_init]).astype(float)
    ...
    def _qaoa_initial_candidates(self, layers: int, prev_best: Optional[np.ndarray], rng) -> list[tuple[str, np.ndarray]]:
        candidates = []
        base = self._base_qaoa_init(layers)
        ...
        if prev_best is not None and self.qaoa_layerwise_warm_start:
            ...
            if target_count >= 2:
                candidates.append(("linear_baseline", base))
            remaining = max(target_count - len(candidates), 0)
            for restart_idx in range(remaining):
            ...
            for restart_idx in range(max(target_count - 1, 0)):
                perturbed = base + rng.normal(0.0, self.qaoa_restart_perturbation, size=2 * layers)
    ...
    def run_qaoa(self):
        ...
        estimated_evals = max(self.qaoa_maxiter * total_planned_restarts, 1)
        progress_plan = self._progress_plan()
        ...
        for stage_index, (active_layers, planned_restart_count) in enumerate(stage_restart_plan, start=1):
            ...
            init_candidates = self._qaoa_initial_candidates(active_layers, best_params, rng)
