#!/usr/bin/env python3
"""Tkinter GUI for the Version 3 QAOA optimizer."""
...
from qaoa_optimizer_v3_core import OptimizationCancelled, OptimizationError, QAOAOptimizerV3


class OptimizerGUIV3:
    def __init__(self, root):
        self.root = root
        self.root.title("QAOA Quantomat - Rapid Quantum Prototyping - by qubit-lab.ch")
    ...
    def _setup_ui(self):
        ...
        ttk.Label(
            header_frame,
            text="QAOA Quantomat - Rapid Quantum Prototyping - by qubit-lab.ch",
            style="Title.TLabel",
        ).grid(row=0, column=1, sticky=tk.W)
        ttk.Label(
            header_frame,
            text="Version 3 notebook engine with GUI controls and live optimization feedback",
        ...
        widget = ttk.Spinbox(override_frame, from_=1, to=1000000, textvariable=self.qaoa_shots_var, width=10)
    ...
    def _write_gui_settings_to_workbook(self, xlsx_path: str):
        ...
        updates = {
            ...
            "qaoa_shots": int(float(self.qaoa_shots_var.get())),
        ...
        *,
        ...
        for df, label in [(classical.head(1), "Best Classical"), (quantum.head(1), "Best Quantum")]:
            if len(df):
                ...
                ax.annotate(
                    label,
                    (row["portfolio_vol"], row["portfolio_return"]),
                    xytext=(8, 8),
        ...
        ax.set_xlabel("Portfolio Volatility", color=self.colors["text"])
        ax.set_ylabel("Portfolio Return", color=self.colors["text"])
    ...
    def _save_risk_return_sharpe_chart(self, optimizer: QAOAOptimizerV3, out_dir: Path) -> Path:
    ...
    def _save_risk_return_qubo_chart(self, optimizer: QAOAOptimizerV3, out_dir: Path) -> Path:
        classical_cmap = mcolors.LinearSegmentedColormap.from_list(
            "classical_qubo",
    ...
    def _save_solver_comparison_chart(self, optimizer: QAOAOptimizerV3, out_dir: Path) -> Path:
    ...
    def _save_qaoa_history_chart(self, optimizer: QAOAOptimizerV3, out_dir: Path) -> Path | None:
    ...
    def _best_row_text(self, row: pd.Series, label: str) -> str:
        ...
        return (
            ...
            f"Return: {row.get('portfolio_return', np.nan):.6f}\n"
            f"Volatility: {row.get('portfolio_vol', np.nan):.6f}\n"
            f"Sharpe ratio: {row.get('sharpe_like', np.nan):.6f}\n"
    ...
    def _build_summary_text(self, optimizer: QAOAOptimizerV3, chart_dir: Path) -> str:
        lines = [
        ...
        if len(optimizer.samples_df):
            positive_quantum = self._positive_probability_quantum_df(optimizer)
            lines.append("")
            lines.append(self._best_row_text(optimizer.samples_df.sort_values("probability", ascending=False).iloc[0], "Best Quantum by Probability"))
            if len(positive_quantum):
                lines.append("")
                lines.append(self._best_row_text(positive_quantum.sort_values("qubo_value").iloc[0], "Best Quantum by QUBO"))
        return "\n".join(lines)
    ...
    def _reconstruct_portfolio_rows(self, optimizer, candidate_row: pd.Series, options_df: pd.DataFrame) -> pd.DataFrame:
        ...
        for pos in selected_idx:
            ...
            row = {
                ...
                "exclusivity_term": candidate_row.get("exclusivity_term", np.nan),
    ...
    def _load_results_from_workbook(self, xlsx_path: str):
        ...
        return SimpleNamespace(
            xlsx_path=path,
            classical_results=read_sheet("Classical_Candidates"),
            samples_df=read_sheet("QAOA_Samples"),
            overview_df=read_sheet("Results_Overview"),
            portfolios_df=read_sheet("Results_Portfolios"),
            solver_comparison_df=read_sheet("Solver_Comparison"),
    ...
    def _show_results_window(self, optimizer: QAOAOptimizerV3):
        chart_dir = self._results_output_dir(str(optimizer.xlsx_path))
        chart_paths = [
            ("Risk / Return - Sharpe Ratio", self._save_risk_return_sharpe_chart(optimizer, chart_dir)),
            ("Risk / Return - QUBO", self._save_risk_return_qubo_chart(optimizer, chart_dir)),
        ...
        classical_cols = self._display_columns(
            ...
            ["bitstring", "qubo_value", "selected_usd", "portfolio_return", "portfolio_vol", "sharpe_like", "budget_gap"],
        )
        quantum_cols = self._display_columns(
            optimizer.samples_df,
            ["bitstring", "probability", "qubo_value", "selected_usd", "portfolio_return", "portfolio_vol", "sharpe_like", "budget_gap"],
        )
        holdings_df = self._build_portfolio_contents_df(optimizer)
    ...
    def _build_optimizer(self, xlsx_path):
        ...
        return QAOAOptimizerV3(
            ...
            qaoa_shots_override=int(self.qaoa_shots_var.get()),
    ...
    def _optimization_worker(self, xlsx_path):
        try:
            ...
            self._log_message("Starting QAOA Quantomat by qubit-lab.ch")
            self._log_message("=" * 70)
            ...
            self._log_message("=" * 70)
            messagebox.showinfo("Success", "Optimization complete. Results written to Excel.")
