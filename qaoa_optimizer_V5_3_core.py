#!/usr/bin/env python3
"""
QAOA Optimizer Core - Version 5.3.

V5.3 builds on V5.2 and starts working directly on the remaining
concentration problem: QAOA can already find good-QUBO states, but in some
regimes it still leaves too much probability mass away from them.
...
All other behaviour is preserved. The class is exposed as ``QAOAOptimizerV53``
and aliased as ``QAOAOptimizerV52`` / ``QAOAOptimizerV51`` / ``QAOAOptimizerV5`` /
``QAOAOptimizerV4`` / ``QAOAOptimizerV3`` so existing callers keep working.
"""
...
class QAOAOptimizerV53:
    """Application-friendly wrapper around the Version 5.3 notebook logic."""
    ...
    def build_qubo(self):
        self._ensure_not_stopped()
        self._log("Building Version 5.3 QUBO problem...")
    ...
    def run_classical_search(self):
        ...
        self._log("Running Version 5.3 classical heuristic...")
    ...
    def _exact_qubo_state_values(self) -> np.ndarray:
        ...
        for idx in range(total_states):
            bitvec = np.array(list(map(int, format(idx, f"0{self.n}b"))), dtype=int)
            values[idx] = float(self.qubo_value(bitvec))
        return values
    ...
    def run_qaoa(self):
        ...
        self._log(f"Running Version 5.3 QAOA (p={self.qaoa_p})...")
    ...
    def generate_results(self):
        self._ensure_not_stopped()
        self._log("Generating Version 5.3 result tables...")
    ...
    def write_results(self):
        self._ensure_not_stopped()
        self._log("Writing Version 5.3 results back to Excel...")
...
# V5.3: class is now QAOAOptimizerV53. Keep V5.2/V5.1/V5/V4/V3 aliases so
# external callers and older scripts continue to work. Importing under any of
# these names within this module picks up the V5.3 fixes automatically.
QAOAOptimizerV52 = QAOAOptimizerV53
QAOAOptimizerV51 = QAOAOptimizerV53
QAOAOptimizerV5 = QAOAOptimizerV53
QAOAOptimizerV4 = QAOAOptimizerV53
QAOAOptimizerV3 = QAOAOptimizerV53
