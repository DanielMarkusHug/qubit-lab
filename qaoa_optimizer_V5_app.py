#!/usr/bin/env python3
"""Tkinter GUI for the Version 5 QAOA optimizer.

V5 is a bug-fix / tuning refresh of the V4 app+core. The GUI is unchanged; the
only app-level edits are:

* Imports now point at ``qaoa_optimizer_V5_core``.
* The driver class is renamed ``OptimizerGUIV5`` (previously
  ``OptimizerGUIV3``).
* The banner / summary strings advertise Version 5.
* Internal type hints reference ``QAOAOptimizerV5``.

All V5-specific algorithmic fixes (scale-aware COBYLA ``rhobeg``/``catol``,
doubled adaptive gamma scale, de-duplicated QAOA seed family, adaptive p=1
presearch gamma window, per-parameter COBYLA iteration budget) live in
``qaoa_optimizer_V5_core.py`` and are driven from the Excel workbook's
``Settings`` sheet, so no new UI controls were added.
"""
...
from qaoa_optimizer_V5_core import OptimizationCancelled, OptimizationError, QAOAOptimizerV5


class OptimizerGUIV5:
    ...
    def _setup_ui(self):
        ...
        ttk.Label(
            header_frame,
            text="Version 5 with scale-aware COBYLA (rhobeg/catol), adaptive p=1 presearch, doubled gamma scale, de-duplicated seed family, and per-parameter iteration budget",
    ...
    def _save_risk_return_sharpe_chart(self, optimizer: QAOAOptimizerV5, out_dir: Path) -> Path:
    ...
    def _save_risk_return_qubo_chart(self, optimizer: QAOAOptimizerV5, out_dir: Path) -> Path:
        classical_cmap = mcolors.LinearSegmentedColormap.from_list(
            "classical_qubo",
    ...
    def _save_risk_return_qubo_quantum_by_qubo_chart(self, optimizer: QAOAOptimizerV5, out_dir: Path) -> Path:
        classical_cmap = mcolors.LinearSegmentedColormap.from_list(
    ...
    def _save_risk_return_qubo_budget_normalized_chart(self, optimizer: QAOAOptimizerV5, out_dir: Path) -> Path:
        classical_cmap = mcolors.LinearSegmentedColormap.from_list(
    ...
    def _save_solver_comparison_chart(self, optimizer: QAOAOptimizerV5, out_dir: Path) -> Path:
    ...
    def _save_qaoa_history_chart(self, optimizer: QAOAOptimizerV5, out_dir: Path) -> Path | None:
    ...
    def _build_summary_text(self, optimizer: QAOAOptimizerV5, chart_dir: Path) -> str:
        lines = [
    ...
    def _collect_circuit_details(self, optimizer) -> dict:
        probe = QAOAOptimizerV5(str(optimizer.xlsx_path), log_callback=None, progress_callback=None)
        probe.load_input()
        probe.build_qubo()
    ...
    def _show_results_window(self, optimizer: QAOAOptimizerV5):
