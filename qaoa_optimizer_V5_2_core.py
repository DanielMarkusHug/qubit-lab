#!/usr/bin/env python3
"""
QAOA Optimizer Core - Version 5.2.

V5.2 builds on V5.1 and fixes a seed-selection bug that silently wasted the
exact p=1 presearch result when the user ran with ``qaoa_multistart_restarts = 1``
and ``qaoa_layerwise_warm_start = False``.
...
All other behaviour is preserved. The class is exposed as ``QAOAOptimizerV52``
and aliased as ``QAOAOptimizerV51`` / ``QAOAOptimizerV5`` / ``QAOAOptimizerV4``
...
class QAOAOptimizerV52:
    """Application-friendly wrapper around the Version 5.2 notebook logic."""
    ...
    def build_qubo(self):
        self._ensure_not_stopped()
        self._log("Building Version 5.2 QUBO problem...")
    ...
    def run_classical_search(self):
        ...
        self._log("Running Version 5.2 classical heuristic...")
    ...
    def _run_exact_p1_presearch(self, make_qaoa_energy, offset: float, qaoa_progress_start: float, qaoa_opt_end: float):
        ...
        if bool(getattr(self, "qaoa_exact_p1_presearch_adaptive", True)):
            ...
            self._log(
                "V5.1 adaptive p=1 presearch gamma window: "
        ...
        if not np.allclose(canonical_seed, np.asarray(best_pair, dtype=float), atol=1e-12, rtol=0.0):
            self._log(
                "V5.1 canonicalized exact p=1 presearch seed via symmetry: "
    ...
    def run_qaoa(self):
        ...
        history_columns += ["energy", "expected_qubo", "best_energy", "best_expected_qubo", "elapsed_sec"]
        ...
        self._log(f"Running Version 5.2 QAOA (p={self.qaoa_p})...")
        ...
        def make_objective_logged(active_layers: int, restart_idx: int, restart_total: int, stage_index: int, stage_total: int, init_type: str):
            ...
            def objective_logged(x):
                ...
                eval_counter += 1
                avg_iter_sec = elapsed / max(eval_counter, 1)
                remaining_iters = max(estimated_evals - eval_counter, 0)
        ...
        for stage_index, (active_layers, planned_restart_count) in enumerate(stage_restart_plan, start=1):
            ...
            for restart_idx, (init_type, x0) in enumerate(init_candidates, start=1):
                ...
                self._log(
                    f"QAOA COBYLA setup: rhobeg={rhobeg_val:.6f}  catol={catol_val:.2e}  "
                    f"maxiter={effective_maxiter}  (gamma_scale={gamma_scale_for_rhobeg:.6f})"
                )
                res = minimize(
                    objective_logged,
                    x0=x0,
            ...
            if active_layers == 1 and self.qaoa_p > 1:
                ...
                if not np.allclose(canonical_stage_params, best_params, atol=1e-12, rtol=0.0):
                    self._log(
                        "V5.1 canonicalized completed p=1 stage params for stable higher-p warm starts: "
    ...
    def generate_results(self):
        self._ensure_not_stopped()
        self._log("Generating Version 5.2 result tables...")
    ...
    def write_results(self):
        self._ensure_not_stopped()
        self._log("Writing Version 5.2 results back to Excel...")
        ...
        summary_items = [
            ...
            ("QAOA exact p1 presearch adaptive (V5.1)", int(bool(getattr(self, "qaoa_exact_p1_presearch_adaptive", False)))),
            ("QAOA exact p1 presearch gamma width factor (V5.1)", float(getattr(self, "qaoa_exact_p1_presearch_gamma_width_factor", 3.0))),
            ("QAOA COBYLA rhobeg setting (V5.1)", str(getattr(self, "qaoa_cobyla_rhobeg", "auto"))),
            ("QAOA COBYLA catol (V5.1)", float(getattr(self, "qaoa_cobyla_catol", 1e-6))),
            ("QAOA gamma scale multiplier (V5.1)", float(getattr(self, "qaoa_gamma_scale_multiplier", 2.0))),
            ("QAOA maxiter per param (V5.1)", int(getattr(self, "qaoa_maxiter_per_param", 30))),
            ("QAOA prefer p1 presearch seed (V5.2)", int(bool(getattr(self, "qaoa_prefer_p1_presearch_seed", True)))),
            (
                "QAOA winning seed (V5.2 final stage)",
                str(getattr(self, "qaoa_last_stage_best_init_type", None) or "n/a"),
            ),
            ("QAOA p1 warm-start canonicalization (V5.1)", 1),
...
# V5.2: class is now QAOAOptimizerV52. Keep V5.1/V5/V4/V3 aliases so external
# callers and older scripts continue to work. Importing under any of these
# names within this module picks up the V5.2 fixes automatically.
QAOAOptimizerV51 = QAOAOptimizerV52
QAOAOptimizerV5 = QAOAOptimizerV52
QAOAOptimizerV4 = QAOAOptimizerV52
QAOAOptimizerV3 = QAOAOptimizerV52
