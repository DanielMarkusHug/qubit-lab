#!/usr/bin/env python3
"""Tkinter GUI for the Version 5.4 QAOA optimizer.

V5.4 keeps the V5.3 GUI layout verbatim and points it at the V5.4 core. All
algorithmic changes live in ``qaoa_optimizer_V5_4_core.py``:
...
The Risk/Return - QUBO Value chart still overlays the three series from
V5.3: Classical top 100 by QUBO, Quantum top 100 by probability, and
Quantum top 100 by QUBO.
"""
...
from qaoa_optimizer_V5_4_core import OptimizationCancelled, OptimizationError, QAOAOptimizerV54


class OptimizerGUIV54:
    ...
    def _setup_ui(self):
        ...
        ttk.Label(
            header_frame,
            text="Version 5.4 fixes the QAOA flat-plateau failure by replacing the zero-padded lift with a ramp schedule and adding COBYLA parameter preconditioning",
    ...
    def _save_risk_return_sharpe_chart(self, optimizer: QAOAOptimizerV54, out_dir: Path) -> Path:
    ...
    def _save_risk_return_qubo_chart(self, optimizer: QAOAOptimizerV54, out_dir: Path) -> Path:
        classical_cmap = mcolors.LinearSegmentedColormap.from_list(
            "classical_qubo",
    ...
    def _save_risk_return_qubo_quantum_by_qubo_chart(self, optimizer: QAOAOptimizerV54, out_dir: Path) -> Path:
        classical_cmap = mcolors.LinearSegmentedColormap.from_list(
    ...
    def _save_risk_return_qubo_budget_normalized_chart(self, optimizer: QAOAOptimizerV54, out_dir: Path) -> Path:
        classical_cmap = mcolors.LinearSegmentedColormap.from_list(
    ...
    def _save_solver_comparison_chart(self, optimizer: QAOAOptimizerV54, out_dir: Path) -> Path:
    ...
    def _save_qaoa_history_chart(self, optimizer: QAOAOptimizerV54, out_dir: Path) -> Path | None:
    ...
    def _build_summary_text(self, optimizer: QAOAOptimizerV54, chart_dir: Path) -> str:
        lines = [
        ...
        if len(optimizer.samples_df):
            ...
            if len(positive_quantum):
                lines.append("")
                best_qubo_label = (
                    "Best Quantum by QUBO (Exact Diagnostic)"
                    if quantum_analysis.get("best_by_qubo_exact")
                    else "Best Quantum by QUBO"
                )
                lines.append(self._best_row_text(quantum_analysis["best_by_qubo"], best_qubo_label))
                lines.append("")
    ...
    def _collect_circuit_details(self, optimizer) -> dict:
        probe = QAOAOptimizerV54(str(optimizer.xlsx_path), log_callback=None, progress_callback=None)
        probe.load_input()
        probe.build_qubo()
    ...
    def _show_results_window(self, optimizer: QAOAOptimizerV54):
...
OptimizerGUIV52 = OptimizerGUIV54
OptimizerGUIV51 = OptimizerGUIV54
OptimizerGUIV5 = OptimizerGUIV54


# V5.4: keep the older GUI aliases importable so launcher scripts continue
# to work unchanged.
OptimizerGUIV53 = OptimizerGUIV54
OptimizerGUIV52 = OptimizerGUIV54
OptimizerGUIV51 = OptimizerGUIV54
OptimizerGUIV5 = OptimizerGUIV54
