"""API-key storage backends for Version 9.
...
class ApiKeyStore:
    ...
    def find_key_by_id(self, key_id: str) -> dict[str, Any] | None:
        raise NotImplementedError
...
class LocalYamlApiKeyStore(ApiKeyStore):
    ...
    def find_key_by_id(self, key_id: str) -> dict[str, Any] | None:
        for candidate in self.key_store.get("keys", []):
            if str(candidate.get("key_id") or "") == str(key_id):
                return _safe_key_record(candidate, store="local")
        return None
...
class FirestoreApiKeyStore(ApiKeyStore):
    ...
    def find_key_by_id(self, key_id: str) -> dict[str, Any] | None:
        snapshot = self.client.collection(Config.FIRESTORE_KEY_COLLECTION).document(str(key_id)).get()
