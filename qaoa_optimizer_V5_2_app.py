#!/usr/bin/env python3
"""Tkinter GUI for the Version 5.2 QAOA optimizer.

V5.2 keeps the V5.1 GUI layout and points it at the V5.2 core. The app-level
edits are narrow:

* Imports now point at ``qaoa_optimizer_V5_2_core``.
* The driver class is renamed ``OptimizerGUIV52``.
* The banner / summary strings advertise Version 5.2.
* Internal type hints reference ``QAOAOptimizerV52``.
* The Risk/Return - QUBO Value chart now overlays a third series --
  Quantum top 100 by QUBO (neon-green hollow diamonds) -- so classical
  best-QUBO, quantum highest-probability, and quantum best-QUBO can be
  compared apples-to-apples on a single plot. This makes it easy to tell
  whether QAOA is (a) missing the good-QUBO region entirely, or (b)
  finding good-QUBO states but not concentrating probability on them.

All V5.2-specific algorithmic fixes (including the presearch-seed selection
fix documented in the core) live in ``qaoa_optimizer_V5_2_core.py`` and are
driven from the Excel workbook's ``Settings`` sheet, so no new UI controls
were added.
"""
...
from qaoa_optimizer_V5_2_core import OptimizationCancelled, OptimizationError, QAOAOptimizerV52


class OptimizerGUIV52:
    ...
    def _setup_ui(self):
        ...
        ttk.Label(
            header_frame,
            text="Version 5.2 adds presearch-seed fix (1-restart runs now actually use the p=1 presearch) and a three-way Risk/Return chart (classical, quantum-by-probability, quantum-by-QUBO)",
    ...
    def _save_risk_return_metric_chart(
        ...
        *,
        ...
        # V5.2: optional third series. When enabled, plot the quantum top 100
        # sorted by QUBO value alongside the usual by-probability series so the
        # user can visually separate "where QAOA concentrated" from "which
        # QAOA samples were actually best for the objective". If the primary
        # quantum series is already by QUBO, overlay would duplicate it; skip
        # in that case.
        quantum_by_qubo = None
        if (
            quantum_qubo_overlay
            and len(optimizer.samples_df)
            and quantum_selection != "qubo"
            and "qubo_value" in optimizer.samples_df.columns
        ):
            quantum_by_qubo = optimizer.samples_df.nsmallest(100, "qubo_value").copy()
        ...
        # V5.2: overlay the quantum top-100 by QUBO series (if requested and
        # distinct from the primary quantum series). Rendered in a single
        # contrasting color without a colorbar to keep the plot readable —
        # the point is to compare *locations* in (vol, return) against the
        # classical-best cloud, not to re-encode QUBO by colour.
        if quantum_by_qubo is not None and len(quantum_by_qubo):
            ax.scatter(
                ...
                marker="D",
                alpha=0.95,
                label="Quantum top 100 by QUBO",
                zorder=3,
            )
            # Annotate the single best-by-QUBO quantum state as well — this is
            # the key diagnostic point: if QAOA's best *sampled* QUBO is close
            # to the classical best, QAOA is finding good states even when the
            # probability distribution itself is diffuse.
            best_row = quantum_by_qubo.iloc[0]
            if x_col in best_row.index and y_col in best_row.index:
                ax.annotate(
                    "Best Quantum (by QUBO)",
    ...
    def _save_risk_return_sharpe_chart(self, optimizer: QAOAOptimizerV52, out_dir: Path) -> Path:
    ...
    def _save_risk_return_qubo_chart(self, optimizer: QAOAOptimizerV52, out_dir: Path) -> Path:
        classical_cmap = mcolors.LinearSegmentedColormap.from_list(
            "classical_qubo",
    ...
    def _save_risk_return_qubo_quantum_by_qubo_chart(self, optimizer: QAOAOptimizerV52, out_dir: Path) -> Path:
        classical_cmap = mcolors.LinearSegmentedColormap.from_list(
    ...
    def _save_risk_return_qubo_budget_normalized_chart(self, optimizer: QAOAOptimizerV52, out_dir: Path) -> Path:
        classical_cmap = mcolors.LinearSegmentedColormap.from_list(
    ...
    def _save_solver_comparison_chart(self, optimizer: QAOAOptimizerV52, out_dir: Path) -> Path:
    ...
    def _save_qaoa_history_chart(self, optimizer: QAOAOptimizerV52, out_dir: Path) -> Path | None:
    ...
    def _build_summary_text(self, optimizer: QAOAOptimizerV52, chart_dir: Path) -> str:
        lines = [
    ...
    def _collect_circuit_details(self, optimizer) -> dict:
        probe = QAOAOptimizerV52(str(optimizer.xlsx_path), log_callback=None, progress_callback=None)
        probe.load_input()
        probe.build_qubo()
    ...
    def _show_results_window(self, optimizer: QAOAOptimizerV52):
...
OptimizerGUIV51 = OptimizerGUIV52
OptimizerGUIV5 = OptimizerGUIV52


def main():
    root = tk.Tk()
    app = OptimizerGUIV52(root)
    root.mainloop()
    return app
