#!/usr/bin/env python3
"""
QAOA Optimizer Core - Version 5.4.

V5.4 fixes a systematic failure mode identified in V5.3: the production
...
1. Zero-padded lift of the p=1 presearch seed to p>1 places the optimizer on
   ...
   COBYLA then "converges" to that plateau. V5.4 exposes a new setting
...
2. COBYLA uses a single ``rhobeg`` for all parameters even though gamma has
   ...
   scale ``~pi/4 ~ 0.8``. One step size cannot be right for both. V5.4
...
1. Exact-mode concentration polish.
   ...
   new settings:
   ``qaoa_concentration_polish_enabled`` (default True),
...
All other behaviour is preserved. The class is exposed as ``QAOAOptimizerV54``
...
class QAOAOptimizerV54:
    """Application-friendly wrapper around the Version 5.4 notebook logic."""
    ...
    def __init__(
        ...
        self.qaoa_exact_qubo_diagnostic_rows = 100
        ...
        self.qaoa_concentration_polish_enabled = True
        ...
        # V5.4: how to lift the p=1 presearch seed to p>1. "ramp" uses a
        ...
        # V5.4: COBYLA parameter preconditioning. When enabled, the objective
        ...
        # V5.4: diagnostic toggle for the per-layer parameter log line at
        # the end of the final stage. Kept on by default.
        self.qaoa_log_final_layer_params = True
        # V5.4: ETA fallback for unobserved later layer stages. A p=4
    ...
    def _apply_runtime_settings(self):
        ...
        self.qaoa_exact_qubo_diagnostic_rows = max(
            0, self._setting_int("qaoa_exact_qubo_diagnostic_rows", 100)
        )
        ...
        # V5.4: lift strategy for the p=1 presearch seed at p>1.
        ...
        # V5.4: COBYLA parameter preconditioning.
    ...
    def build_qubo(self):
        self._ensure_not_stopped()
        self._log("Building Version 5.4 QUBO problem...")
    ...
    def run_classical_search(self):
        ...
        self._log("Running Version 5.4 classical heuristic...")
    ...
    def _lift_p1_seed(self, p1_seed: np.ndarray, target_layers: int) -> np.ndarray:
        ...
        V5.4: supports three strategies via ``qaoa_p1_lift_strategy``.
    ...
    def run_qaoa(self):
        ...
        self._log(f"Running Version 5.4 QAOA (p={self.qaoa_p})...")
        ...
        for stage_index, (active_layers, planned_restart_count) in enumerate(stage_restart_plan, start=1):
            ...
            for restart_idx, (init_type, x0) in enumerate(init_candidates, start=1):
                ...
                #
                # V5.4: add optional preconditioning. When enabled (default)
                ...
                if precondition:
                    ...
                    self._log(
                        f"QAOA COBYLA setup (V5.4 preconditioned): rhobeg={rhobeg_val:.6f}  "
        ...
        # V5.4: emit a clean per-layer dump of the final parameters. If these
        ...
        if bool(getattr(self, "qaoa_log_final_layer_params", True)):
            ...
            self._log(
                f"V5.4 final QAOA parameters  (lift={self.qaoa_p1_lift_strategy}  "
    ...
    def generate_results(self):
        self._ensure_not_stopped()
        self._log("Generating Version 5.4 result tables...")
    ...
    def write_results(self):
        self._ensure_not_stopped()
        self._log("Writing Version 5.4 results back to Excel...")
        ...
        summary_items = [
            ...
            ("QAOA p1 lift strategy (V5.4)", str(getattr(self, "qaoa_p1_lift_strategy", "ramp"))),
            ("QAOA COBYLA preconditioning (V5.4)", int(bool(getattr(self, "qaoa_cobyla_precondition", True)))),
            ("QAOA COBYLA rhobeg preconditioned (V5.4)", float(getattr(self, "qaoa_cobyla_rhobeg_preconditioned", 0.30))),
            ("QAOA ETA layer growth per layer (V5.4)", float(getattr(self, "qaoa_eta_layer_growth_per_layer", 0.40))),
            ("QAOA final gammas (V5.4)", ", ".join(f"{float(v):+.6f}" for v in np.asarray(getattr(self, "best_gammas", np.array([])), dtype=float))),
            ("QAOA final betas (V5.4)", ", ".join(f"{float(v):+.6f}" for v in np.asarray(getattr(self, "best_betas", np.array([])), dtype=float))),
...
# V5.4: class is now QAOAOptimizerV54. Keep V5.3/V5.2/V5.1/V5/V4/V3 aliases so
# external callers and older scripts continue to work. Importing under any of
# these names within this module picks up the V5.4 fixes automatically.
QAOAOptimizerV53 = QAOAOptimizerV54
QAOAOptimizerV52 = QAOAOptimizerV54
QAOAOptimizerV51 = QAOAOptimizerV54
QAOAOptimizerV5 = QAOAOptimizerV54
QAOAOptimizerV4 = QAOAOptimizerV54
QAOAOptimizerV3 = QAOAOptimizerV54
