#!/usr/bin/env python3
"""
QAOA Optimizer Core - Version 6.1.

V6.1 keeps the V6 optimizer behavior and adds a V6.1 GUI entry point with a
scrollable full-circuit diagram. V6 adds fixed/variable asset roles on top of
V5.4. The Assets sheet can keep
...
Historical V5.3/V6.1 concentration-polish settings are still readable from old
workbooks for auditability, but V6.1 now forces both exact and sampled polish
off. The final parameters stay at the plain expected-energy QAOA optimum.
...
All other behaviour is preserved. The class is exposed as ``QAOAOptimizerV61``
and aliased as ``QAOAOptimizerV6`` / ``QAOAOptimizerV53`` / ``QAOAOptimizerV52`` /
``QAOAOptimizerV51`` / ``QAOAOptimizerV5`` / ``QAOAOptimizerV4`` /
``QAOAOptimizerV3`` so existing callers keep working.
"""
...
class QAOAOptimizerV61:
    """Application-friendly wrapper around the Version 6.1 notebook logic."""
    ...
    def __init__(
        ...
        # V6.1: when growing p layerwise, do not zero-pad new layers by
        # default. Zero-padding preserves the old circuit exactly, but it can
        # put the added layer on a flat identity plateau and prevents QAOA
        # probability mass from concentrating on better states. "interp"
        # stretches the previous schedule across the new depth; "repeat"
        # repeats the last known angle; "zero_pad" is legacy/debug only.
        self.qaoa_layerwise_extension_strategy = "interp"
        ...
        # The legacy ``qaoa_maxiter`` still acts as a lower bound; 0 disables
        # the per-parameter multiplier and uses qaoa_maxiter only.
        ...
        # Concentration/CVaR polish is kept as historical metadata only. V6.1
        # now leaves the final parameters at the plain expected-energy QAOA
        # optimum, because polish objectives can move probability mass in ways
        # that are hard to interpret against the canonical QAOA objective.
        ...
        # Historical sampled CVaR polish settings. The polish path is forcibly
        # disabled in runtime settings below; these values are retained only so
        # older workbooks can still be read without errors.
        self.qaoa_sampled_concentration_polish_enabled = False
        self.qaoa_sampled_concentration_polish_alpha = 0.20
        self.qaoa_sampled_concentration_polish_maxiter = 40
        self.qaoa_sampled_concentration_polish_rhobeg = 0.20
        self.qaoa_sampled_concentration_validation_repeats = 3
        ...
        self.qaoa_export_scope = ""
        self.qaoa_best_exported_qubo_scope = ""
        self.qaoa_exact_diagnostic_scope = ""
        ...
        self.qaoa_sampled_concentration_polish_used = False
        self.qaoa_sampled_concentration_polish_cvar_before = np.nan
        self.qaoa_sampled_concentration_polish_cvar_after = np.nan
    ...
    def _apply_runtime_settings(self):
        ...
        layerwise_extension = str(
            self._setting_value("qaoa_layerwise_extension_strategy", self.qaoa_layerwise_extension_strategy)
        ).strip().lower()
        if layerwise_extension not in {"interp", "repeat", "zero_pad"}:
            layerwise_extension = "interp"
        self.qaoa_layerwise_extension_strategy = layerwise_extension
        ...
        # Disabled by design for the canonical V6.1 path. It is a post-QAOA
        # objective, so workbook values must not silently change the optimized
        # parameters away from the expected-energy optimum.
        self.qaoa_concentration_polish_enabled = False
        ...
        # Disabled by design: sampled CVaR post-polish can improve a sampled
        # tail metric while worsening the actual QAOA expected QUBO. Keep the
        # old settings visible in Results_Summary for auditability, but do not
        # let workbook values turn this post-processing back on implicitly.
        self.qaoa_sampled_concentration_polish_enabled = False
        self.qaoa_sampled_concentration_polish_alpha = float(
            np.clip(
                self._setting_float(
                    "qaoa_sampled_concentration_polish_alpha",
                    self.qaoa_sampled_concentration_polish_alpha,
                ),
                1e-6,
                1.0,
            )
        )
        self.qaoa_sampled_concentration_polish_maxiter = max(
            1,
            self._setting_int(
                "qaoa_sampled_concentration_polish_maxiter",
                self.qaoa_sampled_concentration_polish_maxiter,
            ),
        )
        self.qaoa_sampled_concentration_polish_rhobeg = max(
            1e-4,
            self._setting_float(
                "qaoa_sampled_concentration_polish_rhobeg",
                self.qaoa_sampled_concentration_polish_rhobeg,
            ),
        )
        self.qaoa_sampled_concentration_validation_repeats = max(
            1,
            self._setting_int(
                "qaoa_sampled_concentration_validation_repeats",
                self.qaoa_sampled_concentration_validation_repeats,
            ),
        )
    ...
    def load_input(self):
        ...
        self._log(f"Refresh of Data: {self.refresh_with_yfinance}")
    ...
    def build_qubo(self):
        self._ensure_not_stopped()
        self._log("Building Version 6.1 QUBO problem...")
        ...
        if options_df["Annual Volatility"].isna().any():
        ...
        if not np.isfinite(Sigma_assets).all():
            bad_pairs = []
            bad_positions = np.argwhere(~np.isfinite(Sigma_assets))
            for i, j in bad_positions[:20]:
                bad_pairs.append(f"{asset_universe[int(i)]}/{asset_universe[int(j)]}")
            suffix = "" if len(bad_positions) <= 20 else f" and {len(bad_positions) - 20} more"
            raise OptimizationError(
                "AnnualizedCovariance contains missing or non-finite values for option tickers: "
                + ", ".join(bad_pairs)
                + suffix
            )
    ...
    def run_classical_search(self):
        ...
        self._log("Running Version 6.1 classical heuristic...")
        ...
        for _ in range(max(self.random_search_samples, 0)):
            ...
            candidate_dict[bitstring] = {
                ...
                "selection_scope": "classical heuristic candidate pool",
                "probability": np.nan,
    ...
    def _expand_qaoa_params(self, prev_params: np.ndarray, target_layers: int) -> np.ndarray:
        ...
        strategy = str(getattr(self, "qaoa_layerwise_extension_strategy", "interp")).strip().lower()
        if strategy == "zero_pad":
            ...
            gammas[:prev_layers] = prev_gammas
            betas[:prev_layers] = prev_betas
            gammas[prev_layers:] = prev_gammas[-1]
            betas[prev_layers:] = prev_betas[-1]
        else:
            # V6.1 default: stretch the previous layer schedule across the
            # larger depth. This keeps the warm-start close to the lower-p
            # solution while giving every new layer a nonzero mixer/cost angle,
            # avoiding the identity-layer plateau that zero padding caused.
            old_pos = np.linspace(0.0, 1.0, prev_layers)
            new_pos = np.linspace(0.0, 1.0, target_layers)
            gammas = np.interp(new_pos, old_pos, prev_gammas).astype(float)
            betas = np.interp(new_pos, old_pos, prev_betas).astype(float)
        return np.concatenate([gammas, betas]).astype(float)
    ...
    def _effective_qaoa_maxiter(self, active_layers: int) -> int:
        ...
        if maxiter_per_param <= 0:
            return max(1, int(self.qaoa_maxiter))
    ...
    def run_qaoa(self):
        ...
        self._log(f"Running Version 6.1 QAOA (p={self.qaoa_p})...")
        ...
        sample_dev = dev
        qaoa_shots = max(1, int(self.qaoa_shots))
        ...
        def make_qaoa_energy(active_layers: int):
            ...
            if not exact_mode:
                return qml.set_shots(shots=qaoa_shots)(qaoa_energy)
            return qaoa_energy
        ...
        @qml.set_shots(shots=qaoa_shots)
        @qml.qnode(sample_dev, interface="autograd")
        ...
        if best_params is not None and bool(getattr(self, "qaoa_sampled_concentration_polish_enabled", False)):
            alpha = float(getattr(self, "qaoa_sampled_concentration_polish_alpha", 0.20))
            polish_maxiter = max(1, int(getattr(self, "qaoa_sampled_concentration_polish_maxiter", 40)))
            rhobeg_val = max(1e-4, float(getattr(self, "qaoa_sampled_concentration_polish_rhobeg", 0.20)))
            validation_repeats = max(1, int(getattr(self, "qaoa_sampled_concentration_validation_repeats", 3)))
            gamma_scale = max(1e-12, float(self._adaptive_qaoa_gamma_scale()))

            def sampled_cvar_once(params):
                raw_samples = np.asarray(qaoa_samples(np.asarray(params, dtype=float)))
                if raw_samples.ndim == 1:
                    raw_samples = raw_samples.reshape(1, -1)
                bits = raw_samples.astype(float)
                if len(bits) == 0:
                    return np.inf
                values = np.einsum("bi,ij,bj->b", bits, self.Q, bits, optimize=True) + self.constant
                ...
                if len(values) == 0:
                    return np.inf
                keep = max(1, int(np.ceil(float(np.clip(alpha, 1e-6, 1.0)) * len(values))))
                tail = np.partition(values, keep - 1)[:keep]
                return float(np.mean(tail))

            def sampled_cvar_average(params, repeats):
                vals = [sampled_cvar_once(params) for _ in range(int(repeats))]
                return float(np.mean(vals))

            def to_user(x_tilde):
                x_tilde = np.asarray(x_tilde, dtype=float)
                out = np.empty_like(x_tilde)
                out[: self.qaoa_p] = x_tilde[: self.qaoa_p] * gamma_scale
                out[self.qaoa_p :] = x_tilde[self.qaoa_p :]
                return out

            def to_precond(x_user):
                x_user = np.asarray(x_user, dtype=float)
                out = np.empty_like(x_user)
                out[: self.qaoa_p] = x_user[: self.qaoa_p] / gamma_scale
                out[self.qaoa_p :] = x_user[self.qaoa_p :]
                return out

            pre_polish_params = np.asarray(best_params, dtype=float)
            self.qaoa_sampled_concentration_polish_cvar_before = sampled_cvar_average(
                pre_polish_params,
                validation_repeats,
            )
            self._log(
                f"Running V6.1 sampled concentration polish: alpha={alpha:.3f}  "
                f"maxiter={polish_maxiter}  rhobeg={rhobeg_val:.6f}  "
                f"validation_repeats={validation_repeats}  "
                f"shots_per_eval={max(1, int(self.qaoa_shots))}  "
                f"final_export_mode={self.qaoa_mode}  "
                f"sampled_cvar_before={self.qaoa_sampled_concentration_polish_cvar_before:.12f}"
            )

            def sampled_cvar_precond_objective(x_tilde):
                self._ensure_not_stopped()
                return sampled_cvar_once(to_user(x_tilde))

            polish_res = minimize(
                sampled_cvar_precond_objective,
                x0=to_precond(pre_polish_params),
                method="COBYLA",
                options={
                    "maxiter": polish_maxiter,
                    "rhobeg": rhobeg_val,
            ...
            polished_params = to_user(np.asarray(polish_res.x, dtype=float))
            self.qaoa_sampled_concentration_polish_cvar_after = sampled_cvar_average(
                polished_params,
                validation_repeats,
            )
            if (
                np.isfinite(self.qaoa_sampled_concentration_polish_cvar_after)
                and self.qaoa_sampled_concentration_polish_cvar_after
                <= self.qaoa_sampled_concentration_polish_cvar_before
            ):
                best_params = polished_params
                self.qaoa_sampled_concentration_polish_used = True
                self._log(
                    f"Accepted V6.1 sampled concentration polish: "
                    f"sampled_cvar_after={self.qaoa_sampled_concentration_polish_cvar_after:.12f}"
                )
            else:
                self._log(
                    f"Rejected V6.1 sampled concentration polish: "
                    f"sampled_cvar_after={self.qaoa_sampled_concentration_polish_cvar_after:.12f}  "
                    f"(not better than {self.qaoa_sampled_concentration_polish_cvar_before:.12f})"
                )
        ...
        if exact_mode:
            ...
            if diagnostic_count > 0:
                self.qaoa_exact_diagnostic_scope = (
                    f"exact diagnostic: top {diagnostic_count:,} states by QUBO "
                    f"across 2^{int(self.n)} = {len(probs):,} bitstrings"
                )
                self._log(
                ...
                for idx in self._exact_top_qubo_state_indices(diagnostic_count):
                    row = build_exact_row(int(idx), float(probs[idx]), self.qaoa_exact_diagnostic_scope)
                    if row is not None:
            ...
            if fast_exact_probability_path:
                ...
                self.qaoa_export_scope = (
                    ...
                    f"2^{int(self.n)} = {len(probs):,} exact bitstrings "
                    f"(configured qaoa_max_export_rows={int(self.qaoa_max_export_rows):,})"
                )
                self._log(
                ...
                if top_count > 0:
                    ...
                    for rank_pos, idx in enumerate(top_idx, start=1):
                        ...
                        if prob < self.qaoa_min_probability_to_export:
                            break
                        row = build_exact_row(int(idx), prob, self.qaoa_export_scope)
                        if row is not None:
                            q_rows.append(row)
                ...
                if self.qaoa_export_mode == "all_filtered":
                    self.qaoa_export_scope = (
                        f"all exact states passing probability/feasibility filters from "
                        f"2^{int(self.n)} = {len(probs):,} bitstrings"
                    )
                else:
                    self.qaoa_export_scope = (
                        f"top {int(self.qaoa_max_export_rows):,} exact states by {self.qaoa_export_sort_by} "
                        f"from 2^{int(self.n)} = {len(probs):,} bitstrings"
                    )
                self._log(
                ...
                for idx, prob in enumerate(probs):
                    ...
                    row = build_exact_row(int(idx), float(prob), self.qaoa_export_scope)
                    if row is None:
                        continue
            ...
            self.qaoa_export_scope = (
                f"unique sampled states from {total_shots:,} shots "
                f"({len(sample_counts):,} unique before export cap), capped at "
                f"{int(self.qaoa_max_export_rows):,} rows sorted by {self.qaoa_export_sort_by}"
            )
            if total_shots > 0:
                counts_sorted = sorted(sample_counts.values(), reverse=True)
                self.qaoa_max_state_probability = float(counts_sorted[0] / total_shots)
                self.qaoa_top10_probability_mass = float(sum(counts_sorted[:10]) / total_shots)
                ...
                q_rows.append(
                    {
                        ...
                        "selection_scope": self.qaoa_export_scope,
        ...
        if len(self.samples_df):
            if "selection_scope" in self.samples_df.columns:
                scope_values = self.samples_df["selection_scope"].dropna().astype(str)
                if len(scope_values):
                    self.qaoa_export_scope = scope_values.iloc[0]
            top_prob_row = (
            ...
            self.qaoa_best_exported_qubo_scope = str(
                best_qubo_row.get("selection_scope", self.qaoa_export_scope) or self.qaoa_export_scope
            )
    ...
    def generate_results(self):
        self._ensure_not_stopped()
        self._log("Generating Version 6.1 result tables...")
    ...
    def write_results(self):
        self._ensure_not_stopped()
        self._log("Writing Version 6.1 results back to Excel...")
        ...
        summary_items = [
            ...
            ("QAOA exported quantum candidate scope", str(getattr(self, "qaoa_export_scope", "") or "n/a")),
            ("QAOA exported quantum candidate count", int(len(self.samples_df))),
            ("QAOA multistart restarts", int(self.qaoa_multistart_restarts)),
            ("QAOA layerwise warm start", int(self.qaoa_layerwise_warm_start)),
            ("QAOA layerwise extension strategy (V6.1)", str(getattr(self, "qaoa_layerwise_extension_strategy", "interp"))),
            ...
            ("QAOA sampled concentration polish enabled (V6.1)", int(bool(getattr(self, "qaoa_sampled_concentration_polish_enabled", False)))),
            ("QAOA sampled concentration polish used (V6.1)", int(bool(getattr(self, "qaoa_sampled_concentration_polish_used", False)))),
            ("QAOA sampled concentration polish alpha (V6.1)", float(getattr(self, "qaoa_sampled_concentration_polish_alpha", 0.20))),
            ("QAOA sampled concentration polish maxiter (V6.1)", int(getattr(self, "qaoa_sampled_concentration_polish_maxiter", 40))),
            ("QAOA sampled concentration polish rhobeg (V6.1)", float(getattr(self, "qaoa_sampled_concentration_polish_rhobeg", 0.20))),
            ("QAOA sampled concentration validation repeats (V6.1)", int(getattr(self, "qaoa_sampled_concentration_validation_repeats", 3))),
            ("QAOA sampled concentration shots per eval (V6.1)", int(getattr(self, "qaoa_shots", 0))),
            ("QAOA sampled concentration also runs in exact export mode (V6.1)", 0),
            ("QAOA sampled concentration polish CVaR before (V6.1)", float(getattr(self, "qaoa_sampled_concentration_polish_cvar_before", np.nan))),
            ("QAOA sampled concentration polish CVaR after (V6.1)", float(getattr(self, "qaoa_sampled_concentration_polish_cvar_after", np.nan))),
            ...
            ("Decision state space", f"2^{int(self.n)} = {(1 << int(self.n)):,} bitstrings"),
            ...
            ("QAOA best exported QUBO scope", str(getattr(self, "qaoa_best_exported_qubo_scope", "") or "n/a")),
            ...
            ("QAOA exact-diagnostic QUBO scope", str(getattr(self, "qaoa_exact_diagnostic_scope", "") or "n/a")),
...
# V6.1: class is now QAOAOptimizerV61. Keep V6/V5.3/V5.2/V5.1/V5/V4/V3
# aliases so external callers and older scripts continue to work.
QAOAOptimizerV6 = QAOAOptimizerV61
QAOAOptimizerV53 = QAOAOptimizerV61
QAOAOptimizerV52 = QAOAOptimizerV61
QAOAOptimizerV51 = QAOAOptimizerV61
QAOAOptimizerV5 = QAOAOptimizerV61
QAOAOptimizerV4 = QAOAOptimizerV61
QAOAOptimizerV3 = QAOAOptimizerV61
